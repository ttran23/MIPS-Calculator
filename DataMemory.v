...
memory[7952] = 32'h0;
    ...
    //Looks right, I think multiple is written as
    //always @ (posedgeClk, MemRead)
	always @ (posedge Clk, posedge MemRead)
    begin
        if (MemWrite == 1 && Clk == 1)
        begin
			memory[Address[11:2]] <= WriteData;
        end
        else if (MemWrite == 2 && Clk == 1)
        begin
            memory[Address[11:2]][7:0] <= WriteData[7:0];
        end
        else if (MemWrite == 3 && Clk == 1)
        begin
    ...
    // "The reading of memory is not clocked."
    // So maybe have always @ (posedge MemRead) ???
    always @ (negedge Clk)
    begin
        if (MemRead == 1)
        begin
